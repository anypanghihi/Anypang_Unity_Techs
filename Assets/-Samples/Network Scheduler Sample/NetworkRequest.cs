...
public class NetworkRequest : IComparer<NetworkRequest>, IDisposable
{
    ...
    /// 요청 상태
    /// </summary>
    public RequestStatus Status { get; set; } = RequestStatus.Idle;
    ...
    /// 응답 핸들러 목록
    /// </summary>
    private INetworkResponseHandler<NetworkResult> Handler = null;
    ...
    /// 네트워크 요청을 초기화합니다.
    /// </summary>
    public NetworkRequest(string url, float interval, HttpMethod method, Action<NetworkResult> onComplete = null, Action<Exception> onError = null)
    {
        ...
        Method = method ?? HttpMethod.Get; // 기본값 설정
    ...
    /// 응답 핸들러를 추가합니다.
    /// </summary>
    public NetworkRequest AddHandler(INetworkResponseHandler<NetworkResult> handler)
    {
        Handler = handler;
    ...
    public void ProcessResponse(NetworkResult result)
    {
        ...
        {
            // 핸들러 처리
            if (Handler != null && Handler.CanHandle())
            {
                try
                {
                ...
                {
                    Debug.LogError($"[Network] 핸들러 처리 중 오류 발생: {handlerEx.Message}");
                    // 핸들러 오류가 전체 처리를 중단시키지 않도록 함
                }
            }
        }
        catch (Exception ex)
        {
            Debug.LogError($"[Network] 응답 처리 중 오류 발생: {ex.Message}");
            Status = RequestStatus.HandleFailed;
            OnError?.Invoke(ex);
        }
    }
